...
Les données ont été télécharger 
...
Nous avons uniquement les read issu d'Illumina --> on va évaluer la qualité de reads
On affiche les profil de qualité des Read1 en avant du premier et du deuxième read issus de la variable crée précédémment FnFs
```{r}
plotQualityProfile(fnFs[1:2])
```

On affiche les profil de qualité des Read2 en arrière du premier et du deuxième read issus de la variable crée précédémment FnRs
```{r}
plotQualityProfile(fnRs[1:2])
```
On va filtrer les variables contenant les reads 1 et 2. À partir de ces filtrages on attribue des variable et des noms associées 
...
head(out)
```
On obtient le nombre de bases (les nucléotides) qui sont gardées une fois la filtration effectuées ici que pour les R1. Pour le reads F3D30 S188 R1  001 :7793 nt pour le read avant filtration et 7113 nt après filtration. 680 nt ont été enlevé : les primers ont été supprimés et les nucléotides pas bons. 
...
But : Le modèle d'erreur de DADA2  permet identier les positions avec une forte probabilité d'erreur et par la suite changer avec la base la plus probable, qui ressemble donc plus a la séquence majoritaire proche abondande

On crée les variables : 
-errF : recoit le modèle d'erreur paramétrique par la méthode LearnErrors pour les read avant filtré (tronqué)

Ici Dada2 calcule un modèle d'erreur à partir des données de séquençages. On applique cette méthode sur les reads forward (Fs) = R1 et reverse (Rs) = R2
...
-ligne noire: taux d'erreur estimé après convergence de l'algorithme d'apprentissage machine. 
-ligne rouge: taux d'erreur attendu selon la définition nominale du Q-score.

Les taux d'erreur estimés (ligne noire) correspondent bien aux taux observés (points), et les taux d'erreur diminuent avec l'augmentation de la qualité comme prévu. Tout semble raisonnable et nous procédons avec confiance
...
```
Calcul pour séquences composées de séquences uniques 
...
Inspecter la deuxième étagère du premier placard de dada2 (deuxième échantillon)
```{r}
dadaFs[[2]]
```
Nous avons fini le filtrage des reads. 
...
- objet seqtab : une table avec en ligne le nombre échantillon, en colonne les séquences elle-même à l'intérieur ou on observer la séquence dans l'échantillon 
...
Avec le primer F(pour les forwards), on crée la séqunece complémentaire, mais pour x ou y, l'élongation s'arrête 
On aura le 16S  qui n'aura pas bouger et un autre fragment simple brin plus court. Lors du cycle suivant en PCR, on va avoir un des fragment 16S (le rouge et l'incomplet) qui vont s'hydrider sur un autre fragment 16S comme un primer et donc continuer élongation (verte) pour donner un truc hybride à la fin qui sera le début du rouge et a la fin du vert
...
Appliquer à seqtab  et transferer à une nouvelle variable seqtab.nochim

-> 1/5, les 293 unique mais qui représente plus de séquence dans le jeu de donnée
...
## Evaluer l'a précision de l'assignation taxonomique 
```{r}
